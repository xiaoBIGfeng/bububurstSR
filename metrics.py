...
class AlignedL2(nn.Module):
    ...
    def forward(self, pred, gt, burst_input):
        ...
        with torch.no_grad():
            flow = self.alignment_net(pred / (pred.max() + 1e-6), gt / (gt.max() + 1e-6))
            # # 在调用 alignment_net 之前检查 pred 和 gt
            # pred_max = pred.max()
            # gt_max = gt.max()
            # print(f"pred max: {pred_max}, gt max: {gt_max}")

            # pred_norm = pred / (pred_max + 1e-6)
            # gt_norm = gt / (gt_max + 1e-6)
            # print(f"pred_norm shape: {pred_norm.shape}, gt_norm shape: {gt_norm.shape}")

            # # 调用 alignment_net
            # flow = self.alignment_net(pred_norm, gt_norm)
            # print(f"flow shape: {flow.shape}, device: {flow.device}, dtype: {flow.dtype}")
        ...
        # print(f"gt shape: {gt.shape}, device: {gt.device}")
        # print(f"burst_input shape: {burst_input.shape}, device: {burst_input.device}")
