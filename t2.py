...
parser.add_argument('--result_dir', default='../log/log_burstomrer/Track_1/results', type=str, help='Directory for results')
# parser.add_argument('--weights', default='../logs\Track_2\saved_model\model_latest.pth', type=str, help='Path to pre-trained weights')
parser.add_argument('--weights', default='D:/downloads/model.ckpt', type=str, help='Path to weights')


args = parser.parse_args()
writer = SummaryWriter('./logs_real_full/6to7')
##################################################################################################################
...
class BurstSR_Test_Network():
    ...
    def test(self):               
        ...
        test_dataset = BurstSRDataset(root="D:/downloads/burstsr_dataset",  split='val', burst_size=14, crop_sz=80, random_flip=False)
        ...
        for i, data in tqdm(enumerate(test_data_loader),total=len(test_data_loader)):
            ...
            # print('Evaluation Measures for Burst {:d} ::: PSNR is {:0.3f}, SSIM is {:0.3f} \n'.format(i, PSNR_temp, SSIM_temp))
