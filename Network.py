...
def to_3d(x): # einops库函数，按照字符串pattern指定的模式来重新排列多维张量的维度
...
class feature_alignment(pl.LightningModule):
    ...
    def offset_gen(self, x):
        ...
        # torch.chunk(x, 3, dim=1) 将输入张量 x 在第1维（特征维度）上分割成3份。
        # 假设 x 的形状是 [B, C, H, W]，其中 C 是通道数，那么 torch.chunk 会将 C 个通道分割成3组，每组包含 C/3 个通道。
    ...
    def forward(self, x, prev_offset_feat=None):
        ...
        B, f, H, W = x.size()
        # x(b,c,h,w)-->x[0]:(c,h,w)-->>unsqueeze(0):(1,c,h,w)
        ref = x[0].unsqueeze(0)
        ref = torch.repeat_interleave(ref, B, dim=0)

        offset_feat = self.bottleneck(torch.cat([ref, x], dim=1)) # 一个卷积层用于计算偏移量
        ...
        aligned_feat = self.deform(x, offset, mask) # 看DeformConv2d forward方法的input
